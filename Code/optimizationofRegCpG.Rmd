...
cpg_reg <- function(outcome, data, name, Topn){
  ...
  return(coef)
   ...
   kable(head(outcome_lm, Topn), 
         caption = paste("Top10 CpGs for ", name, " by p.value",
...
cpg_reg(pfas_male$birth_weight, pfas_male, "birth_weight", 15)
...
unlist(lapply(Outcomes, function(x){
  cpg_reg(pfas_male[, x], pfas_male, c(x, "Male"), 15)
})
)
...
lapply(Outcomes[3:9], function(x){
  cpg_reg(log10(pfas_male[, x]), pfas_male, c(x, "Male log10"), 15)
})
...
## log10 of feMale
lapply(Outcomes, function(x){
  cpg_reg(log10(pfas_female[, x]), pfas_female, c(x, "Female log10"), 15)
})
